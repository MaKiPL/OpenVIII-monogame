        ...
        private class IGMData_Mag_ST_A_D_Slots : IGMData_Slots<Saves.CharacterData>
        {
            ...
            public override void CheckMode(bool cursor = true) =>
                CheckMode(0, Mode.Mag_ST_A, Mode.Mag_ST_D,
                    Junction != null && (Junction.GetMode().Equals(Mode.Mag_ST_A) || Junction.GetMode().Equals(Mode.Mag_ST_D)),
                    Junction != null && (Junction.GetMode().Equals(Mode.Mag_Pool_ST_A) || Junction.GetMode().Equals(Mode.Mag_Pool_ST_D)),
                    cursor);
            ...
            public override bool Inputs_OKAY()
            {
                if (!BLANKS[CURSOR_SELECT])
                {
                    base.Inputs_OKAY();
                    BackupSetting();
                    Junction.SetMode(CURSOR_SELECT == 0 ? Mode.Mag_Pool_ST_A : Mode.Mag_Pool_ST_D);
                    return true;
                }
                return false;
            }
            ...
            protected override void PageLeft() => Junction.SetMode(Mode.Mag_Stat);

            protected override void PageRight() => Junction.SetMode(Mode.Mag_EL_A);
