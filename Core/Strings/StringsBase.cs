        ...
        public abstract class StringsBase
        {
                ...
                else
                {
                    ...
                    while (br.BaseStream.Position < fpos.max)
                    {
                        ...
                        else
                        {
                            if (b != 0xFFFF)
                            {
                                Files.sPositions[PointerStart].Add(new FF8StringReference(Archive,filename, b + StringStart));
                                last = b;
                            }
                            else
            ...
            protected void Get_Strings_ComplexStr(BinaryReader br, string filename, uint key, List<uint> list)
            {
                ...
                for (uint p = 0; p < fPaddings.Length; p += 2)
                {
                    ...
                    Files.sPositions[key].Add(new FF8StringReference(Archive,filename, (uint)br.BaseStream.Position));
                    //entry contains possible more than one string so I am scanning for null
                    while (br.BaseStream.Position + 1 < stop)
                    {
                        byte b = br.ReadByte();
                        if (b == 0) Files.sPositions[key].Add(new FF8StringReference(Archive, filename, (uint)br.BaseStream.Position));
                    }
                }
            }
                ...
                for (uint p = 0; p < fPaddings.Length; p++)
                {
                    ...
                    if (br.BaseStream.Position + 4 < br.BaseStream.Length)
                    {
                        ...
                        for (int i = 0; i < count && br.BaseStream.Position + 2 < br.BaseStream.Length; i++)
                        {
                            ...
                            {
                                c += fpad;
                                Files.sPositions[key].Add(new FF8StringReference(Archive, filename, (uint)c));
                            }
                        }
                    }
                }
            ...
            protected void simple_init()
            {
                ...
                using (BinaryReader br = new BinaryReader(ms))
                {
                    ...
                    Files = new Stringfile(new Dictionary<uint, List<uint>>(1), new List<Loc>(1) { new Loc { seek = 0, length = uint.MaxValue } });
