    ...
    public partial class Module_main_menu_debug
    {
        private partial class IGM_Junction
        {
            private class IGMData_Mag_EL_A_Values : IGMData
            {
                ...
                {
                    byte[] spell = new byte[] {
                            source.Stat_J[Kernel_bin.Stat.EL_Atk]
                        };
                    ...
                    for (int i = 0; i < spell.Length; i++)
                        ...
                        {
                            int t = total[(Kernel_bin.Element)flag] + ((Kernel_bin.MagicData[spell[i]].J_Val[Kernel_bin.Stat.EL_Atk] * Memory.State.Characters[Character].Magics[spell[i]]) / 100);
                ...
                public override bool Update()
                {
                    if (Memory.State.Characters != null)
                    {
                        int _nag = 0;
                        int _pos = 0;
                        Dictionary<Kernel_bin.Element, byte> oldtotal = (Slots.GetPrevSetting() != null) ? getTotal(Slots.GetPrevSetting(), out Enum[] availableFlagsarray) : null;
